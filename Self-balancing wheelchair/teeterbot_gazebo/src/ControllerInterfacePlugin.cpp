...
namespace gazebo
{
//set the initial class parameters to begin.
...
{
  // Gazebo setup using wheelchair model
  model_ = model;
...
// receiving updates from gazebo world by passing update function with its input
  update_connection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&ControllerInterfacePlugin::OnUpdate, this, _1));

  // Load SDF parameters,pub_gound_truth: set true to broadcast a TF transform from Gazebo `world` frame to TeeterBot's `base_footprint` frame.
  ...
  // Auto-reset parameters,auto_reset_orientation: set true, and the Gazebo plugin will detect when TeeterBot falls down and automatically reset it to vertical orientation.
...
//detecting proper control mode.
  ...
  // ROS setup
  //creating two dc motor for each wheel.
  ...
  if (torque_mode_) {
    ...
    //creating two motorcontroller for each wheel's torque.
  ...
  if (speed_mode_) {
    ...
    //creating two motorcontroller for each wheel's speed.
...
//create topics including Float64 messages.
  ...
  //two subscribers to get the cmd_topic data and update in via passing recvMotorCmd function.
 ...
 //advertise the nudge serviceand pass the nudgecb arguments to get them later.
...
//std messages created in order to transfer and publish the data.
...
void ControllerInterfacePlugin::OnUpdate(const common::UpdateInfo &info)
{
 ...
 // publish transform from map to base_footprint in order to see displacement in Rviz
  tf::StampedTransform map_transform;
  map_transform.child_frame_id_ = "base_footprint";
  map_transform.frame_id_ = "map";
  map_transform.stamp_.fromSec(info.simTime.Double());
#if GAZEBO_MAJOR_VERSION >= 9
  map_transform.setOrigin(tf::Vector3(pose.Pos().X(), pose.Pos().Y(), 0.0));
#else
  map_transform.setOrigin(tf::Vector3(0.0, 0.0, 0.0));
#endif
  map_transform.setRotation(tf::Quaternion(0.0, 0.0, sin(0.5 * yaw), cos(0.5 * yaw)));
  broadcaster_.sendTransform(map_transform);
  ...
  if (is_nudging_) {
    ...
    body_link_->AddLinkForce(nudge_force_*10, nudge_offset_);
  } else {
#if GAZEBO_MAJOR_VERSION >= 9
...
#else
    ground_truth_transform.setOrigin(tf::Vector3(0.0, 0.0, 0.0));
#endif
...
//get the nudge force and duration from the service
